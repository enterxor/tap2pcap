...
if __name__ == '__main__':
    ...
    if False:
        ...
        outbuf= win32file.AllocateReadBuffer(1000)
        win32file.DeviceIoControl(handle, TAP_IOCTL_SET_MEDIA_STATUS, bytes([1,0,0,0]),outbuf, None)
        win32file.DeviceIoControl(handle, TAP_IOCTL_CONFIG_TUN,
                                  b'\x0a\x03\x00\x01\x0a\x03\x00\x00\xff\xff\xff\x00',outbuf, None)
    
    fle = open('temporary.bin','w');

    pc= pcap.Pcap('foo.pcap')
    while True:
